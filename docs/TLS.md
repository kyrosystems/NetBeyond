...
На XP нет `bcrypt.dll`, а в `msvcrt.dll` нет `_vsnprintf_s`; build-tree патчи переводят entropy на CryptoAPI `CryptGenRandom` и secure CRT ветку MinGW на стандартный `vsnprintf`. На XP нет также `inet_pton`/`inet_ntop` в `WS2_32.dll`. CMake передаёт `_WIN32_WINNT=0x0501` и `WINVER=0x0501` всей Mbed TLS-сборке. `x509_crt.c` сам определяет `MBEDTLS_TEST_SW_INET_PTON` для XP; задавать его через `-D` нельзя, потому что `-Werror` отклоняет переопределение. CI запрещает импорты `bcrypt.dll`, `_vsnprintf_s`, `_vsprintf_s`, `inet_pton`, `inet_ntop` и прикладывает `imports.txt`.
